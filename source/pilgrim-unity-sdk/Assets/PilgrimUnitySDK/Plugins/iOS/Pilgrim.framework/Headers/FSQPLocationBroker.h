...
@protocol FSQPLocationSubscriber, FSQPRegionMonitoringSubscriber;

#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 80000
@protocol FSQPVisitMonitoringSubscriber;
#endif

#pragma mark - FSQPLocationBroker interface

/**
 * Manager for location events application-wide. Subscribers must implement the
 * _FSQPXXXSubscriber_ protocol and themselves to the list of subscribers in order to receive
 * notifications from the broker.
 */
NS_SWIFT_NAME(LocationBroker)
@interface FSQPLocationBroker : NSObject <CLLocationManagerDelegate>
/**
 ...
 *
 * Normally creates and returns an instance of FSQPLocationBroker. If you would like to use a custom broker subclass
...
/**
 * If you would like to use a custom subclass of FSQPLocationBroker in your app, you can set its class using this
 * method. All future calls to [FSQPLocationBroker shared] will return the shared method of that class instead.
 *
 * If you specify a class that is not a subclass of FSQPLocationBroker this method will do nothing.
 *
 * @warning If you want to use this method to change the broker class, you must call it before
 * the first call to @c shared.
 *
 * @param locationBrokerSubclass A subclass of FSQPLocationBroker to use instead of the base implementation.
 */
 ...
 */
- (void)addLocationSubscriber:(NSObject<FSQPLocationSubscriber> *)locationSubscriber NS_REQUIRES_SUPER;
 ...
 * reflected if you access the locationSubscribers property.
 */
- (void)removeLocationSubscriber:(NSObject<FSQPLocationSubscriber> *)locationSubscriber NS_REQUIRES_SUPER;
 ...
 */
- (void)addRegionMonitoringSubscriber:(NSObject<FSQPRegionMonitoringSubscriber> *)regionSubscriber NS_REQUIRES_SUPER;
 ...
 * reflected if you access the regionSubscribers property.
 */
- (void)removeRegionMonitoringSubscriber:(NSObject<FSQPRegionMonitoringSubscriber> *)regionSubscriber NS_REQUIRES_SUPER;
 ...
 *
 * @param region The region whose state you want to know. This object must be an instance of one of the standard region
 * subclasses provided by Map Kit. You cannot use this method to determine the state of custom regions you define yourself.
 */
 ...
 */
- (void)addVisitSubscriber:(NSObject<FSQPVisitMonitoringSubscriber> *)visitSubscriber NS_REQUIRES_SUPER;
 ...
 * reflected if you access the visitSubscribers property.
 */
- (void)removeVisitSubscriber:(NSObject<FSQPVisitMonitoringSubscriber> *)visitSubscriber NS_REQUIRES_SUPER;
...
#pragma mark - FSQPLocationSubscriber Protocol

/**
 * Bitmask configuration options for the FSQPLocationSubscriber protocol
 *
 * Subscribers should bitwise OR the options they want together.
 */
typedef NS_OPTIONS(NSUInteger, FSQPLocationSubscriberOptions) {
    /**
     ...
     */
    FSQPLocationSubscriberShouldRequestContinuousLocation    = (1 << 0),
     ...
     * from the system, regardless of which subscriber "caused" those updates.
     */
    FSQPLocationSubscriberShouldMonitorSLCs                  = (1 << 1),
     ...
     */
    FSQPLocationSubscriberShouldReceiveErrors                = (1 << 2),
     ...
     * requesting because of other subscribers, but not actually affect what the broker requests from the system.
     *
     ...
     */
    FSQPLocationSubscriberShouldReceiveAllBrokerLocations    = (1 << 3),
     ...
     */
    FSQPLocationSubscriberShouldRunInBackground              = (1 << 4),
} NS_SWIFT_NAME(LocationSubscriberOptions);

/**
 * The _FSQPLocationSubscriber_ protocol.
 *
 ...
 * subscriberOptions
 *
 * There is no guarantee changing the return values will affect _FSQPLocationBroker_ behavior if
 ...
 */
NS_SWIFT_NAME(LocationSubscriber)
@protocol FSQPLocationSubscriber<NSObject>
 ...
 *
 * See the documentation for FSQPLocationSubscriberOptions to get information on the available options.
 *
 ...
 */
@property (nonatomic, readonly) FSQPLocationSubscriberOptions locationSubscriberOptions;

/**
 * If the subscriber options include @c FSQPLocationSubscriberShouldRequestContinuousLocation then this accuracy
 ...
 *
 * If your subscriber options property includes the FSQPLocationSubscriberShouldReceiveErrors you must implement this
...
#pragma mark - FSQPVisitMonitoringSubscriber Protocol

NS_SWIFT_NAME(VisitMonitoringSubscriber)
@protocol FSQPVisitMonitoringSubscriber <NSObject>
...
#pragma mark - FSQPRegionMonitoringSubscriber Protocol

/**
 * The protocol for subscribing to Region Monitoring events.
 *
 * The following properties **must be** KVO compliant OR **implementors must call**
 * [[FSQPLocationBroker shared] refreshRegionMonitoringSubscribers] after changing their the return values
 * in order for changes to take place:
 *
 * monitoredRegions
 *
 ...
 * values change for KVO compliant properties.
 */
NS_SWIFT_NAME(RegionMonitoringSubscriber)
@protocol FSQPRegionMonitoringSubscriber<NSObject>

/**
 * List of regions the subscriber wants to monitor.
 *
 * The FSQPRegionMonitoringSubscriber must maintain its own list of monitored regions.
 *
 * If the property is KVO-compliant, the broker will automatically update its state when changes occur. Otherwise
 * you must manually call @c refreshLocationSubscribers on the broker to have your changes reflected.
 */
