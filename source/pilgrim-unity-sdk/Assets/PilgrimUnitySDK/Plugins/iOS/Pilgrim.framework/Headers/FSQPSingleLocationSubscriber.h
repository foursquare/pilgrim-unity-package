//
//  FSQPSingleLocationSubscriber.h
//  PilgrimSDK
//
...
@class FSQPSingleLocationRequestConfig;

#import "FSQPLocationBroker.h"
 ...
 */
typedef void (^FSQPSingleLocSubCompletionBlock)(BOOL didSucceed, CLLocation *location, NSNumber *elapsedTime, NSError *error);
 ...
 * It uses FSQPLocationBroker to try to find a location which matches your parameters and then return the result
 * via a block.
 */
NS_SWIFT_NAME(SingleLocationSubscriber)
@interface FSQPSingleLocationSubscriber : NSObject<FSQPLocationSubscriber>
 ...
 */
@property (nonatomic, readonly) FSQPSingleLocSubCompletionBlock onCompletion;
 ...
 *
 * @param desiredAccuracy           The accuracy to ask the system location manager for.
 *
 * @param maximumAcceptableAccuracy The first received location with a horizontal accuracy less than or equal to this value will stop the
 *                                  subscriber and be returned via the completion block.
 *
 * @param maximumAcceptableRecency  If the location broker already has a location under your max accuracy
 *                                  it will immediately return if the location is no older than this time interval.
 *
 * @param cutoffTimeInterval        If an acceptable location hasn't been received within this time, the subscriber
 *                                  will stop listening and call your completion block.
 *
 * @param shouldRunInBackground     If YES, the subscriber will keep trying to get a location when backgrounded.
 *                                  May have not have the intended effect unless your app has background location entitlements.
 *                                  If NO, the subscriber will stop and call your completion block when the app is backgrounded.
 *
 * @param onCompletion              This block will be called with the first location the subscriber finds that
 *                                  meets your requirements. If the subscriber does not receive an acceptable location within the
 *                                  cutoffTime you set it will send you the best accuracy location it could get in that time.
 *                                  (for the most recent location, you should check FSQPLocationBroker instead).
 *                                  This will also be called if the app is backgrounded before the subscriber could finish and
 *                                  shouldRunInBackground was not YES, or if the system location manager encounters an error.
 *
...
+ (instancetype)startWithDesiredAccuracy:(CLLocationAccuracy)desiredAccuracy
        ...
        maximumAcceptableLocationRecency:(NSTimeInterval)maximumAcceptableRecency
                   ...
                   shouldRunInBackground:(BOOL)shouldRunInBackground
                            onCompletion:(FSQPSingleLocSubCompletionBlock)onCompletion;
 ...
 * stop listening for updates and pass the location to the callback you provide.
 *
 * @param config                    A configuration object with all parameters required to configure the location subscriber
 *
 * @param onCompletion              This block will be called with the first location the subscriber finds that
 *                                  meets your requirements. If the subscriber does not receive an acceptable location within the
 *                                  cutoffTime you set it will send you the best accuracy location it could get in that time.
 ...
 *                                  shouldRunInBackground was not YES, or if the system location manager encounters an error.
 */
+ (instancetype)startWithConfig:(FSQPSingleLocationRequestConfig *)config onCompletion:(FSQPSingleLocSubCompletionBlock)onCompletion;
...
/**
 *  Unavailable. Use a start method instead.
 */
